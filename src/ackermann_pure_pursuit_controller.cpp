  ...
  : state(INACTIVE), stuck(new StuckDetector)
{
  ...
  follow_path_server_->registerGoalCallback(boost::bind(&Ackermann_Pure_Pursuit_Controller::followPathGoalCallback, this));
  follow_path_server_->registerPreemptCallback(boost::bind(&Ackermann_Pure_Pursuit_Controller::followPathPreemptCallback, this));
...
bool Ackermann_Pure_Pursuit_Controller::configure()
{
  ...
  stateSubscriber     = nh.subscribe("state", 10, &Ackermann_Pure_Pursuit_Controller::stateCallback, this, ros::TransportHints().tcpNoDelay(true));
  drivetoSubscriber   = nh.subscribe("driveto", 10, &Ackermann_Pure_Pursuit_Controller::drivetoCallback, this);
  drivepathSubscriber = nh.subscribe("drivepath", 10, &Ackermann_Pure_Pursuit_Controller::drivepathCallback, this);
  cmd_velSubscriber   = nh.subscribe("cmd_vel", 10, &Ackermann_Pure_Pursuit_Controller::cmd_velCallback, this, ros::TransportHints().tcpNoDelay(true));
  cmd_velTeleopSubscriber = nh.subscribe("cmd_vel_teleop", 10, &Ackermann_Pure_Pursuit_Controller::cmd_velTeleopCallback, this, ros::TransportHints().tcpNoDelay(true));
  speedSubscriber     = nh.subscribe("speed", 10, &Ackermann_Pure_Pursuit_Controller::speedCallback, this);
  poseSubscriber      = nh.subscribe("robot_pose", 10, &Ackermann_Pure_Pursuit_Controller::poseCallback, this);
...
bool Ackermann_Pure_Pursuit_Controller::updateRobotState(const nav_msgs::Odometry& odom_state)
{
...
void Ackermann_Pure_Pursuit_Controller::poseCallback(const ros::MessageEvent<geometry_msgs::PoseStamped>& event)
{
...
void Ackermann_Pure_Pursuit_Controller::stateCallback(const nav_msgs::OdometryConstPtr& odom_state)
{
...
void Ackermann_Pure_Pursuit_Controller::drivetoCallback(const ros::MessageEvent<geometry_msgs::PoseStamped>& event)
{
...
bool Ackermann_Pure_Pursuit_Controller::driveto(const geometry_msgs::PoseStamped& goal, double speed)
{
  reset();
...
void Ackermann_Pure_Pursuit_Controller::drivepathCallback(const ros::MessageEvent<nav_msgs::Path>& event)
{
...
bool Ackermann_Pure_Pursuit_Controller::pathToBeSmoothed(const std::deque<geometry_msgs::PoseStamped>& transformed_path, bool fixed_path)
{
...
bool Ackermann_Pure_Pursuit_Controller::drivepath(const nav_msgs::Path& path)
{
  reset();
  ...
  if(!options.is_fixed)
  {
    ...
    std::for_each(smooth_path.begin() + 1, smooth_path.end(), boost::bind(&Ackermann_Pure_Pursuit_Controller::addLeg, this, _1, options.desired_speed));
  ...
  if(!legs.empty())
    ...
    ROS_WARN("[vehicle_controller] Ackermann_Pure_Pursuit_Controller::drivepath produced empty legs array.");

  return true;
}


bool Ackermann_Pure_Pursuit_Controller::createDrivepath2MapTransform(tf::StampedTransform & transform, const nav_msgs::Path& path)
{
...
void Ackermann_Pure_Pursuit_Controller::cmd_velCallback(const geometry_msgs::Twist& velocity)
{
...
void Ackermann_Pure_Pursuit_Controller::cmd_velTeleopCallback(const geometry_msgs::Twist& velocity)
{
  if (follow_path_server_->isActive()){
...
void Ackermann_Pure_Pursuit_Controller::speedCallback(const std_msgs::Float32& speed)
{
  mp_.commanded_speed = speed.data;
}

void Ackermann_Pure_Pursuit_Controller::stopVehicle()
{
...
void Ackermann_Pure_Pursuit_Controller::followPathGoalCallback()
{
...
void Ackermann_Pure_Pursuit_Controller::followPathPreemptCallback()
{
  stopVehicle();
  move_base_lite_msgs::FollowPathResult result;
  result.result.val = move_base_lite_msgs::ErrorCodes::PREEMPTED;
...
void Ackermann_Pure_Pursuit_Controller::addLeg(const geometry_msgs::PoseStamped& pose, double speed)
{
  Leg leg;
...
bool Ackermann_Pure_Pursuit_Controller::reverseAllowed()
{
...
bool Ackermann_Pure_Pursuit_Controller::reverseForced()
{
  return false;
  if (follow_path_server_->isActive()) {
...
void Ackermann_Pure_Pursuit_Controller::reset()
{
  state = INACTIVE;
  current = 0;
  final_twist_trials = 0;
...
void Ackermann_Pure_Pursuit_Controller::update()
{
  if (state < DRIVETO) return;
  ...
  Ackermann_Pure_Pursuit_Controller::computeMoveCmd(robot_control_state);
...
void Ackermann_Pure_Pursuit_Controller::stop()
{
...
void Ackermann_Pure_Pursuit_Controller::computeMoveCmd(RobotControlState control_state){
  double lookahead_distance = control_state.carrot_distance;

  const double alpha = control_state.error_2_path_angular;
...
double Ackermann_Pure_Pursuit_Controller::exponentialSpeedControll(){
